...
class ServiceController extends Controller
{
    ...
    public function expiring()
    {
        ...
        // Get date time 2 days from now
        $twoDaysFromNow = Carbon::now()->addDays(2);

        // Debug info
        echo "<!-- Current time: " . $now . " -->";
        echo "<!-- Two days from now: " . $twoDaysFromNow . " -->";

        // Get services that will end within the next 2 days
        // This means: end_datetime > now AND end_datetime <= (now + 2 days)
        $expiringServices = Service::where('end_datetime', '>', $now)
                                 ->where('end_datetime', '<=', $twoDaysFromNow)
                                 ->with(['serviceCategory', 'serviceType'])
                                 ->get();

        // Debug info
        echo "<!-- Services found: " . $expiringServices->count() . " -->";

        // Calculate remaining time for each service
        foreach ($expiringServices as $service) {
            $endTime = Carbon::parse($service->end_datetime);
            // Debug info
            echo "<!-- Service: " . $service->name . " ends at: " . $endTime . " -->";

            // Calculate remaining seconds
            $service->remaining_time = $now->diffInSeconds($endTime, false);
            echo "<!-- Remaining time: " . $service->remaining_time . " seconds -->";

            // Calculate available seats
            $service->available_seats = $service->total_available_seats - $service->bookings()->count();
        }


        return view('public.index', compact('expiringServices'));
    }
}
